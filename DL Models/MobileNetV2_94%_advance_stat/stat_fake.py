...
SAMPLE_RATE = 16000
DURATION = 3.0
N_MELS = 64  # Reduced for lightweight processing
HOP_LENGTH = 512
N_FFT = 1024  # Adjusted for noisy audio

# Enhanced data augmentation for noisy/low-quality audio
augment = Compose([
    AddGaussianNoise(min_amplitude=0.005, max_amplitude=0.03, p=0.7),  # Increased for noise robustness
    TimeStretch(min_rate=0.7, max_rate=1.3, p=0.6),  # Wider range for variability
    PitchShift(min_semitones=-5, max_semitones=5, p=0.6)  # Wider pitch shift
])
...
# Training function with t-SNE at intervals
...
def evaluate_model(model, test_loader, device):
    ...
    f1_scores = f1_score(true, preds, average=None)  # Per-class F1
    ...
    cm_analysis = {
        ...
        'Analysis': (
            ...
            f"False positives ({cm[0, 1]}) indicate REAL audio misclassified as FAKE, "
            f"potentially due to noise or similar spectral patterns. "
            f"False negatives ({cm[1, 0]}) suggest FAKE audio misclassified as REAL, "
            f"which could be addressed with more diverse augmentation."
        )
    }
...
# Gradio interface for real-time mic input
